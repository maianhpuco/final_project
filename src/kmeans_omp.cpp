...
// RGB to Lab conversion helper functions (same as CPU version)
...
void KMeansOMP::rgbToLab(float r, float g, float b, float& l, float& a, float& b_out) {
...
void KMeansOMP::labToRgb(float l, float a, float b_in, float& r, float& g, float& blue) {
    // Convert Lab to XYZ
...
std::vector<std::vector<float>> KMeansOMP::initializeCentroids(
    const Image& image,
    int k,
...
std::vector<std::vector<float>> KMeansOMP::updateCentroids(
    const Image& image,
    ...
    std::vector<std::vector<std::vector<float>>> threadCentroids(
        numThreads, std::vector<std::vector<float>>(k, std::vector<float>(3, 0.0f))
    );
    std::vector<std::vector<int>> threadCounts(
        numThreads, std::vector<int>(k, 0)
    );
    ...
    #pragma omp parallel
    {
        ...
        for (int y = 0; y < image.height; y++) {
            for (int x = 0; x < image.width; x++) {
                ...
                threadCentroids[threadId][cluster][0] += pixelColor[0];
                threadCentroids[threadId][cluster][1] += pixelColor[1];
                threadCentroids[threadId][cluster][2] += pixelColor[2];
    ...
    for (int t = 0; t < numThreads; t++) {
        for (int i = 0; i < k; i++) {
            newCentroids[i][0] += threadCentroids[t][i][0];
            newCentroids[i][1] += threadCentroids[t][i][1];
            newCentroids[i][2] += threadCentroids[t][i][2];
            counts[i] += threadCounts[t][i];
        }
    }
...
double KMeansOMP::computeError(
    ...
    // Use reduction for thread-safe summation
...
KMeansResult KMeansOMP::segment(
    const Image& image,
    int k,
    ColorSpace colorSpace,
    int maxIterations,
    ...
    std::cout << "Using OpenMP with " << actualThreads << " thread(s)" << std::endl;
    ...
    // Create segmented image (parallelize this too)
